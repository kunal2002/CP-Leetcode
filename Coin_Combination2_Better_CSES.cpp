...
int rec(vector<vector<int>>& dp, vector<int>& vec,int sum,int j)
{
   if(j == 0)
   {
      if(sum % vec[j] == 0)
   ...
   int not_T = rec(dp,vec,sum,j-1);
   int take = 0;
   if(vec[j] <= sum)
   {
      take = rec(dp,vec,sum-vec[j],j);
   }
   return dp[sum][j] = take + not_T;
}
