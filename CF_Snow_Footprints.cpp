...
{	
	...
	string str;
	cin >> str;
	int r = find(all(str),'R') - str.begin() + 1;
	int l = find(all(str),'L') - str.begin() + 1;
	if(count(all(str),'R') == 0)
	{
		cout << l << " " << l-1;
		return;
	}
	else if(count(all(str),'L') == 0)
	{
		reverse(all(str));
		int idx = find(all(str),'R') - str.begin() + 1;
		cout << n - idx + 1 << " " << n - idx + 2;
		return;
	}
	cout << r << " " << l-1;
}
