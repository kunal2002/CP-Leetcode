...
int rec(vector<vector<int>>& dp, vector<int>& vec,int sum,int checks,int j)
{
   if(sum == checks)
   {
        dp[sum][j] = 1;
   ...
   for(int i = j; i < vec.size(); i++)
   {
        if(sum+vec[i] <= checks)
        {
            ans = (ans%MOD + rec(dp,vec,sum+vec[i],checks,i)%MOD) % MOD;
        }
        else if(i+1 < vec.size())
        {
            ans = (ans%MOD + rec(dp,vec,sum,checks,i+1)%MOD) % MOD;
        } 
   }
   return dp[sum][j] = ans;
}
